...
class BodyTemplateItem(BaseModel):
    ...
    input: str = Field(...,
                       description='Input variable name to map from runtime vars')
...
class ThirdServiceCreateRequest(BaseModel):
    service_name: str = Field(..., max_length=64,
                              example='Cloudflare URL Scanner')
    base_url: str = Field(
        ...,
    ...
    def validate_mapping_json(cls, validate):
        if validate is not None and isinstance(validate, dict) and validate:
            ...
            missing_fields = [
                field for field in required_fields if field not in validate]
            if missing_fields:
                raise ValueError(
                    f"Fields are required: {', '.join(missing_fields)}")
        return validate
...
class ThirdServiceExecuteRequest(BaseModel):
    vars: dict = Field(...,
                       description='Runtime variables to resolve body template')
