...
class AccessKeyCreateRequest(BaseModel):
    key_name: Optional[str] = Field(None, max_length=64, description="Name for the access key")
    expired_at: Optional[datetime] = Field(None, description="Expiration date for the key")
...
class AccessKeyAdminCreateRequest(BaseModel):
    ...
    key_name: Optional[str] = Field(None, max_length=64, description="Name for the access key")
    usage_limit: Optional[int] = Field(None, ge=0, description="Usage limit for the key (null = unlimited)")
    expired_at: Optional[datetime] = Field(None, description="Expiration date for the key")
...
class UsageStatQuery(BaseModel):
    ...
    month: Optional[int] = Field(None, ge=1, le=12, description="Month to query (1-12)")
