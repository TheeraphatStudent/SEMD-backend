...
class Settings(BaseSettings):
    ...
    # Auth settings
    auth_secret_key: str = config.get(
        'AUTH', 'SECRET_KEY', fallback='your-secret-key-here')
    auth_algorithm: str = config.get('AUTH', 'ALGORITHM', fallback='HS256')
    auth_access_token_expire_minutes: int = config.getint(
        'AUTH', 'ACCESS_TOKEN_EXPIRE_MINUTES', fallback=15)
    auth_refresh_token_expire_days: int = config.getint(
