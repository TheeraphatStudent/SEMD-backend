"""
Redis worker for processing ML prediction results.
This worker listens to ml_result_queue and caches results for the API to retrieve.
...
class MLResultWorker:
    ...
    def process_result(self, result_data: dict) -> bool:
        try:
            ...
            logger.info(f"Processing {job_type} result: {job_id} (status: {status})")
            ...
            if result_data.get("prediction"):
                ...
                logger.info(f"  Class: {pred.get('class')}, Confidence: {pred.get('confidence')}")
